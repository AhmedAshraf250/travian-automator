@php
    ...
    $hasScheduledTroopOrders = $quickTroopOrders->isNotEmpty();
    $celebrationEnabled = (bool) ($settings?->celebration_enabled ?? false);
    $celebrationUseHeroResources = (bool) ($settings?->celebration_use_hero_resources ?? false);
    ...
    $troopControlClasses = $automationControlsDisabled
        ? $pausedSignalClasses
        : ($hasScheduledTroopOrders
            ? 'border-amber-700 bg-amber-400 text-amber-950 shadow-inner ring-1 ring-amber-700/25'
            : ($quickTroopUnits !== []
                ? 'border-indigo-700 bg-indigo-600 text-white shadow-inner ring-1 ring-indigo-700/30 hover:bg-indigo-700'
                : 'border-slate-300 bg-slate-100 text-slate-400 shadow-sm'));
    ...
    <div class="grid gap-2 rounded-lg border {{ $isVillagePaused ? 'border-amber-400 bg-amber-50' : 'border-[var(--color-line)] bg-[var(--color-panel)]' }} px-3 py-2 shadow-sm xl:grid-cols-[minmax(10.5rem,0.75fr)_minmax(0,3.35fr)_auto] xl:items-center">
        <div class="min-w-0">
            ...
            <div class="relative mt-1 flex flex-wrap items-center gap-1.5 text-[11px] text-[var(--color-muted)]">
                ...
                <button type="button" @click="openControl = openControl === 'troops' ? null : 'troops'"
                    @disabled($automationControlsDisabled)
                    class="relative inline-flex h-7 w-7 items-center justify-center rounded-md border text-[11px] font-extrabold transition {{ $troopControlClasses }}"
                    title="T - Troop command panel: indigo means trainable units are ready; amber means orders are pending">
                    T
                    @if ($quickTroopOrders->isNotEmpty())
                        <span class="absolute -right-1.5 -top-1.5 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-slate-800 px-1 text-[9px] text-white">{{ $quickTroopOrders->count() }}</span>
                    @endif
                </button>
                ...
                <div x-cloak x-show="openControl === 'celebrations'" @click.outside="openControl = null" @keydown.escape.window="openControl = null"
                    ...
                    <button type="button" wire:click="toggleVillageCelebrationHeroResources({{ $village->id }})"
                        @disabled($automationControlsDisabled)
                        class="mt-3 flex w-full items-center justify-between gap-3 rounded-lg border px-2.5 py-2 text-left transition {{ $celebrationUseHeroResources ? 'border-amber-500/40 bg-amber-50 text-amber-950' : 'border-[var(--color-line)] bg-[var(--color-panel-alt)] text-[var(--color-muted)]' }}"
                        title="Use Hero resources for a celebration when village resources are insufficient">
                        <span><strong class="block text-[11px]">Hero resources if short</strong><span class="block text-[10px] font-normal">Verified shortage only</span></span>
                        <span class="relative inline-flex h-5 w-9 items-center rounded-full {{ $celebrationUseHeroResources ? 'bg-amber-500' : 'bg-slate-300' }}">
                            <span class="absolute h-4 w-4 rounded-full bg-white shadow-sm transition {{ $celebrationUseHeroResources ? 'right-0.5' : 'left-0.5' }}"></span>
                        </span>
                    </button>
                ...
                <div x-cloak x-show="openControl === 'troops'" @click.outside="openControl = null" @keydown.escape.window="openControl = null"
                    class="absolute left-0 top-8 z-50 w-80 max-w-[calc(100vw-2rem)] rounded-lg border border-[var(--color-line)] bg-[var(--color-panel)] p-3 text-xs shadow-[0_18px_55px_rgba(15,23,42,0.18)]">
                    <div class="flex items-center justify-between gap-2">
                        <span class="font-semibold text-[var(--color-ink)]">Troop orders</span>
                        <button type="button" wire:click="$dispatch('dashboard-open-village-troops', { villageId: {{ $village->id }} })" class="text-[11px] font-semibold text-[var(--color-accent)] hover:underline">Open full view</button>
                    </div>

                    @if ($quickTroopUnits !== [])
                        @php
                            $quickOptions = collect($quickTroopUnits)->map(fn (array $unit): array => [
                                ...$unit,
                                'icon_url' => asset($unit['icon_path']),
                            ])->values()->all();
                        @endphp
                        <div class="mt-3" x-data="{
                            units: @js($quickOptions), selectedUnitId: @js($quickOptions[0]['unit_id'] ?? null), quantity: 1, listOpen: false, submitting: false,
                            get selected() { return this.units.find((unit) => unit.unit_id === Number(this.selectedUnitId)) ?? null },
                            submit() {
                                if (! this.selected || this.quantity < 1 || this.submitting) return;
                                this.submitting = true;
                                this.$wire.queueQuickTroopOrder(Number(this.selectedUnitId), Number(this.quantity))
                                    .then(() => { this.quantity = 1 }).finally(() => { this.submitting = false });
                            },
                        }">
                            <div class="grid grid-cols-[minmax(0,1fr)_4.5rem] gap-2">
                                <div class="relative" @click.outside="listOpen = false">
                                    <button type="button" @click="listOpen = ! listOpen" class="flex h-full min-h-8 w-full items-center gap-1.5 rounded-md border border-[var(--color-line-strong)] bg-[var(--color-panel)] px-2 py-1 text-left text-xs text-[var(--color-ink)]">
                                        <template x-if="selected"><img :src="selected.icon_url" alt="" class="h-6 w-6 rounded bg-white object-contain p-0.5 ring-1 ring-black/5"></template>
                                        <span class="min-w-0 flex-1 truncate" x-text="selected ? selected.english : 'Choose unit'"></span><span>▾</span>
                                    </button>
                                    <div x-cloak x-show="listOpen" class="absolute inset-x-0 top-full z-20 mt-1 max-h-48 overflow-y-auto rounded-md border border-[var(--color-line-strong)] bg-[var(--color-panel)] p-1 shadow-xl">
                                        <template x-for="unit in units" :key="unit.unit_id">
                                            <button type="button" @click="selectedUnitId = unit.unit_id; listOpen = false" class="flex w-full items-center gap-2 rounded px-1.5 py-1.5 text-left hover:bg-[var(--color-panel-alt)]">
                                                <img :src="unit.icon_url" alt="" class="h-7 w-7 rounded bg-white object-contain p-0.5 ring-1 ring-black/5"><span class="min-w-0 flex-1 truncate font-semibold" x-text="unit.english"></span>
                                            </button>
                                        </template>
                                    </div>
                                </div>
                                <input type="number" min="1" max="9999" x-model.number="quantity" class="rounded-md border border-[var(--color-line-strong)] bg-[var(--color-panel)] px-2 py-1.5 text-xs text-[var(--color-ink)]" />
                            </div>
                            <button type="button" @click="submit" :disabled="! selected || quantity < 1 || submitting"
                                class="mt-2 w-full rounded-md bg-[var(--color-accent)] px-3 py-1.5 text-xs font-semibold text-[var(--color-accent-contrast)] disabled:opacity-45"><span x-show="! submitting">Queue · 1 minute to cancel</span><span x-cloak x-show="submitting">Queuing…</span></button>
                        </div>
                    @else
                        <p class="mt-3 rounded-md bg-[var(--color-panel-alt)] px-2.5 py-2 text-[11px] leading-4 text-[var(--color-muted)]">No unit is ready to train. Open the full view and refresh the military state.</p>
                    @endif

                    @if ($quickTroopOrders->isNotEmpty())
                        <div class="mt-3 space-y-1 border-t border-[var(--color-line)] pt-2">
                            @foreach ($quickTroopOrders as $order)
                                @php
                                    $quickOrderUnit = collect($quickTroopUnits)->firstWhere('unit_id', $order->unit_id);
                                @endphp
                                <div wire:key="quick-troop-order-{{ $order->id }}" class="flex items-center gap-2 rounded-md bg-[var(--color-panel-alt)] px-2 py-1.5">
                                    <span class="min-w-0 flex-1 truncate font-semibold text-[var(--color-ink)]">{{ $quickOrderUnit['english'] ?? 'Unit '.$order->unit_id }} × {{ $order->requested_quantity }}</span>
                                    <span class="text-[10px] text-[var(--color-muted)]">{{ $order->execute_after->diffForHumans() }}</span>
                                    <button type="button" wire:click="cancelQuickTroopOrder({{ $order->id }})" class="font-semibold text-rose-700">Cancel</button>
                                </div>
                            @endforeach
                        </div>
                    @endif
            ...
            <div class="flex min-w-0 flex-wrap items-center gap-2">
                <div class="flex min-w-0 flex-wrap items-center gap-1.5">
                    ...
                    @php
                        ...
                        $constructionFinishLabel = !empty($constructionEntry['finish_label'])
                            ...
                            : null;
                        $constructionTimerKey = sha1(json_encode([
                            $village->id,
                            'construction',
                            $loop->index,
                            $constructionEntry['building_name'] ?? null,
                            $constructionEntry['target_level'] ?? null,
                            $constructionEndsAtTimestamp,
                        ]));
                        if ($constructionFinishLabel === '00:00') {
                            $constructionFinishLabel = null;
                        }
                    @endphp
                    <span wire:key="construction-timer-{{ $constructionTimerKey }}"
                        class="inline-flex max-w-full items-center gap-1.5 rounded-md border px-2 py-1 font-semibold shadow-sm"
                        ...
                        @if ($constructionRemainingSeconds !== null) x-data="{
                                ...
                                syncQueued: false,
                                init() {
                                    this.tick($store.dashboardClock.now);
                                },
                                tick(now) {
                                    this.remaining = Math.max(0, Math.ceil((this.endsAt - now) / 1000));
                            ...
                            }" x-effect="tick($store.dashboardClock.now)" @endif>
                ...
                <div class="flex min-w-0 flex-wrap items-center gap-1.5">
                    ...
                    @php
                        ...
                        $movementTimerKey = sha1(json_encode([
                            $village->id,
                            'movement',
                            $loop->index,
                            $movementKind,
                            $movementLabel,
                            $movementEndsAtTimestamp,
                        ]));
                    @endphp
                    <span wire:key="movement-timer-{{ $movementTimerKey }}"
                        class="inline-flex items-center gap-1.5 rounded-md border px-2 py-1 font-semibold shadow-sm"
                        ...
                        @if ($movementRemainingSeconds !== null) x-data="{
                            ...
                            }" x-effect="tick($store.dashboardClock.now)"
                            @endif>
