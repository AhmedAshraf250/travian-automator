...
test('sync account overview stores villages and resource state from dorf1 html', function () {
    ...
    expect($village?->population)->toBe(52);
    expect($village?->resourceState?->wood)->toBe(1736);
    ...
    expect($village?->runtimeState?->troop_slots[1] ?? null)->toBe(3);
    expect($village?->runtimeState?->incoming_attack_count)->toBe(0);
    expect($village?->runtimeState?->outgoing_movement_count)->toBe(1);
    expect($village?->runtimeState?->construction_entries)->toHaveCount(2);
    expect($village?->buildings()->where('is_under_construction', true)->count())->toBe(2);
...
test('sync account overview can authenticate through the modern login api flow', function () {
    ...
    app()->bind(AccountSessionFactory::class, function () use ($account, $loginHtml, $dorf1Html) {
        return new class($account, (string) $loginHtml, (string) $dorf1Html) implements AccountSessionFactory
        {
            public function __construct(
                protected Account $account,
            ...
            public function for(Account $account): AccountSession
            {
                return new class($this->account, $this->loginHtml, $this->dorf1Html) implements AccountSession
                {
                    /**
                    ...
                    public function postJson(string $uri, array $payload, array $options = []): SessionResponse
                    {
                        ...
                        expect($payload['w'])->toBe('1920:1200');
